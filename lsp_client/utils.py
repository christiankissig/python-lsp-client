...
def parse_content_type(content_type_string: str | None):
    """
    ...
    if content_type_string is not None:
        ...
        if content_type != DEFAULT_CONTENT_TYPE:
            raise ValueError(
                    f"Unsupported content type in {content_type_string}")
        if (
            len(content_type_parts) > 1
            and content_type_parts[1].strip().startswith('charset=')
        ):
